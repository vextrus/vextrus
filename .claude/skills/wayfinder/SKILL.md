...
**An unattended session may still work a HITL ticket — for facts, never for decisions.** `/grilling` already assigns it the expensive half: *"if a fact can be found by exploring the environment, look it up rather than asking me."* So do that, write what you found into the ticket under its own heading, sharpen the questions the facts sharpened, then **stop at the first decision**: never append `## Resolution`, never set `Status: closed`. Clear your own claim and leave the ticket open — open with the reading done is what this state is, and it needs no new field. An absent or unrecognised type counts as HITL. Enforced in CI, not on your honour: a HITL ticket closed without a matching attended claim on `main` is refused (ADR-0015).
