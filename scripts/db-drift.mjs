#!/usr/bin/env node
/**
 * Drift detector (ADR-0002): db/migrations on disk against the __migrations ledger in the
 * database. Exit 0 clean, 1 drift, 2 unreachable. `--json` for machine-readable output.
 * Drift presents as an application fault (docs/lessons/); this makes it a one-command diagnosis.
 */
...
try {
  ...
  if (ledger.length > 0) {
    inDb = (await sql`SELECT name FROM __migrations ORDER BY name`).map((r) => r.name);
  }
} catch (err) {
...
if (json) {
  ...
  if (unapplied.length) console.log(`UNAPPLIED (run pnpm db:migrate):\n  ${unapplied.join("\n  ")}`);
  if (unknown.length)
    console.log(`UNKNOWN in db ledger (not on disk — wrong branch or deleted migration):\n  ${unknown.join("\n  ")}`);
}
