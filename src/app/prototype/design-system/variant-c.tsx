...
export function VariantC({ lang }: { lang: "en" | "bn" }) {
  ...
  const isBn = lang === "bn";
  const t = item ? queueText(item, lang) : null;
  ...
  return (
    <div
      ...
      style={{
        ...tokens,
        fontFamily: isBn
          ? 'var(--font-bangla, "Noto Sans Bengali"), ui-sans-serif, sans-serif'
          : 'ui-sans-serif, "Segoe UI", system-ui, sans-serif',
        lineHeight: isBn ? 1.7 : 1.3,
      }}
    >
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-[#6a6a66]">
              {isBn ? bn.project : project.name} · {project.drawingSet}
            </p>
            <h1
              className="mt-1 text-2xl font-semibold tracking-tight"
              style={{ lineHeight: isBn ? 1.6 : 1.15 }}
            >
              {isBn
                ? `${absenceSummary.cellsTotal}টি class × kind ঘরের মধ্যে ${absenceSummary.cellsUnresolved}টির কোনো সারি নেই, কোনো কারণও নেই`
                : `${absenceSummary.cellsUnresolved} of ${absenceSummary.cellsTotal} class × kind cells have no line and no cause`}
            </h1>
            <p className="mt-1 max-w-[70ch] text-[0.9375rem] text-[#4a4a46]">
              {isBn
                ? `${absenceSummary.cellsWithLines}টি সারি প্রস্তুত, ${absenceSummary.cellsExcluded}টি কারণসহ ঘোষিত বর্জন। বাকি সবই নীরবতা — একমাত্র নিন্দনীয় অবস্থা।`
                : `${absenceSummary.cellsWithLines} produced lines, ${absenceSummary.cellsExcluded} are declared exclusions with a named cause. Everything else is silence — the only condemned state.`}
            </p>
          </div>
          ...
          <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div>
              <p className="font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-[#6a6a66]">
                ...
                {item.blocking && (
                  <span style={{ color: "var(--proto-refuse)" }}>
                    {" "}
                    · ▮ {isBn ? bn.ui.blocking : "blocks ingestion"}
                  </span>
                )}
              </p>
              <h2 className="mt-1 text-xl font-semibold" style={{ lineHeight: isBn ? 1.6 : 1.2 }}>
                {t?.headline}
              </h2>
              <p className="mt-2 max-w-[68ch] text-[0.9375rem] leading-relaxed text-[#3a3a36]">
                {t?.detail}
              </p>
              <p className="mt-3 max-w-[68ch] border-l-2 border-[#1b1b1a] pl-3 text-[0.8125rem]">
                {isBn ? bn.ui.machine : "machine proposes"} — {t?.proposal}
              </p>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <span className="text-[0.75rem] text-[#6a6a66]">
                  {isBn
                    ? `${bn.ui.act(item.subjects)} — যে গ্রানুলারিটিতে করা হয়েছে`
                    : `recorded as one act with ${item.subjects} subject${item.subjects === 1 ? "" : "s"} — the granularity performed`}
                </span>
              </div>
            </div>
      ...
      <section className="mx-auto mt-6 max-w-[110rem] border-t-2 border-[#1b1b1a] bg-white px-8 py-6">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h2 className="text-lg font-semibold">{isBn ? bn.bill : project.bill}</h2>
          <p className="text-[0.6875rem] uppercase tracking-[0.14em] text-[#6a6a66]">
            {isBn ? bn.ui.subtotalOnly : "measured-scope subtotal only"}
          </p>
        </div>
        <table className="mt-3 w-full border-collapse text-[0.8125rem]">
          <thead>
            <tr className="border-b border-[#c9c9c2] text-left font-mono text-[0.6875rem] uppercase tracking-[0.1em] text-[#6a6a66]">
              ...
              <th className="w-32 py-1 pr-3 text-right font-normal">{isBn ? bn.ui.amount : "amount"}</th>
              <th className="w-56 py-1 pl-3 font-normal">
              ...
              <tr key={l.itemNo} className="border-b border-[#eaeae4] align-top">
                ...
                <td className={`py-2 text-right pr-3 ${figures}`}>
                  {l.amount ? (
                    ...
                    <span className="text-[#6a6a66]">{isBn ? bn.ui.unpriced : "unpriced"}</span>
                  )}
                </td>
        ...
        <h3 className="mt-6 border-t border-[#c9c9c2] pt-3 text-[0.6875rem] uppercase tracking-[0.14em] text-[#6a6a66]">
        ...
        <p className="mt-3 text-[0.75rem] text-[#6a6a66]">
          {isBn
            ? `${bn.ui.signedFor}: ${project.surveyor}. ${bn.noGrandTotal}`
            : `Signed for the measured scope above: ${project.surveyor}. No grand total prints under incomplete coverage.`}
        </p>
      </section>

      {/* The document as it prints, in Bangla — prose leading beside the dense table above it. */}
      {isBn && (
        <section className="mx-auto mt-6 max-w-[110rem] border-t border-[#c9c9c2] bg-white">
          <BanglaCertificate />
        </section>
      )}
    </div>
