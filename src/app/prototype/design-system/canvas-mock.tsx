...
export function CanvasMock({
  ...
  return (
    <figure className="relative h-full w-full overflow-hidden" style={{ background: ground }}>
      ...
      {/* One owner for the bottom-left gutter: the raster note used to be drawn
          inside the SVG at world scale and collided with this caption (and with
          the dimension string) wherever a short canvas box rendered both. */}
      {(caption || raster) && (
        <figcaption
          className="pointer-events-none absolute bottom-2 left-3 space-y-0.5 font-mono text-[0.6875rem] tracking-wide"
          style={{ color: faint }}
        >
          {raster && (
