/**
 * PROTOTYPE — ticket 10. Document presentation of money and quantity, held
 * *inside* the prototype: a real surface would need this in `src/core` (only
 * the spine is importable by the queue, the bill and the PDF alike, and
 * `eslint.config.js` bans cross-module deep imports), but where it lands is
 * part of what the ticket has not yet ruled. Nothing here is production code.
 *
